...
class Template {
	...
	var $compile_dir			= "cache/compile";	// the directory to store the compiled files in
	...
	var $error					= true;//2007-11-27 add by ¿ÝÄ¾
	var $da_error_reporting		= "<?php error_reporting(iCMS_TPL_BUG?E_ALL ^ E_NOTICE:0);!defined('iCMS') && exit('What are you doing?');?>\n";
...
//		$this->_error_level = $this->debugging ? E_ALL ^ E_NOTICE : 0;
		...
		if (strpos($output, $this->_sl_md5) !== false){
			preg_match_all('!' . $this->_sl_md5 . '{_run_iCMS (.*)}' . $this->_sl_md5 . '!U',$output,$_match);
			foreach($_match[1] as $value){
				$arguments = unserialize($value);
				$output = str_replace($this->_sl_md5 . '{_run_iCMS ' . $value . '}' . $this->_sl_md5, call_user_func_array('iCMS_' . $arguments['module'], array((array)$arguments, $this)), $output);
			}
		}
...
//		error_reporting($this->_error_level);
		...
		$this->error && $output = str_replace($this->da_error_reporting,'',$output);
...
//	12:42 2008-8-10
	function _replace_path($output,$file){
		...
		//url()
		$output = preg_replace("/url\({$pattern}\)/", "url({$template_path}\\1)", $output);
		//background
...
//		error_reporting($this->_error_level);
	...
	function _run_iCMS($arguments){
		if ($this->cache){
			return $this->_sl_md5 . '{_run_iCMS ' . serialize((array)$arguments) . '}' . $this->_sl_md5;
		}else{
